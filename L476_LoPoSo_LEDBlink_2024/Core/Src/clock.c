...
/**
  ...
  *            Flash Latency(WS)              = 4
  */
void SystemClock_Config_80M()   // note Antho : c'est la config de l'expe 1. Faire de même pour les autres expe
{
...
void SystemClock_expe2()   // note Antho : c'est la config de l'expe 1. Faire de même pour les autres expe
{
	/* MSI configuration and activation */
	LL_RCC_MSI_Enable();			// normalement il est deja enabled
	while	(LL_RCC_MSI_IsReady() != 1)	// c'est pour le cas ou on l'aurait change
